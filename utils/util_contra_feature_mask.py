...
def saveFeatureMaps(net, data_loader, epoch):
    ...
    saveimgs(data, dir='./contra_feature_mask_result/images/00_input_Img',epoch=epoch)

    acts = extract_layers(net, data.to(device))
    saveimgs(acts[0], dir='./contra_feature_mask_result/images/01_cb1_gabor',epoch=epoch)
    saveimgs(acts[1], dir='./contra_feature_mask_result/images/02_cb1_scc',epoch=epoch)
    saveimgs(acts[2], dir='./contra_feature_mask_result/images/03_cb1_conv1',epoch=epoch)
    saveimgs(acts[3], dir='./contra_feature_mask_result/images/04_cb1_conv2',epoch=epoch)
    saveimgs(acts[4], dir='./contra_feature_mask_result/images/05_cb2_gabor',epoch=epoch)
    saveimgs(acts[5], dir='./contra_feature_mask_result/images/06_cb2_scc',epoch=epoch)
    saveimgs(acts[6], dir='./contra_feature_mask_result/images/07_cb2_conv1',epoch=epoch)
    saveimgs(acts[7], dir='./contra_feature_mask_result/images/08_cb2_conv2',epoch=epoch)
    saveimgs(acts[8], dir='./contra_feature_mask_result/images/09_cb3_gabor',epoch=epoch)
    saveimgs(acts[9], dir='./contra_feature_mask_result/images/10_cb3_scc',epoch=epoch)
    saveimgs(acts[10], dir='./contra_feature_mask_result/images/11_cb3_conv1',epoch=epoch)
    saveimgs(acts[11], dir='./contra_feature_mask_result/images/12_cb3_conv2',epoch=epoch)
...
def saveGaborFilters(net, epoch):
    ...
    '''
    if not os.path.exists('./contra_feature_mask_result/images/gaborfilters'):
        os.makedirs('./contra_feature_mask_result/images/gaborfilters')
    ...
    for o in range(channel_out):
        for i in range(channel_in):
            ...
            im.save('./contra_feature_mask_result/images/gaborfilters/%d_cb1_%03d.png'%(epoch, o))
    ...
    for o in range(channel_out):
        for i in range(channel_in):
            ...
            img = (img - img.min())/(img.max()-img.min()+1e-10)*255
            im = Image.fromarray(img.astype("uint8")).convert('L')
            im.save('./contra_feature_mask_result/images/gaborfilters/%d_cb2_%03d.png'%(epoch, o))
    ...
    for o in range(channel_out):
        for i in range(channel_in):
            ...
            img = (img - img.min())/(img.max()-img.min()+1e-10)*255
            im = Image.fromarray(img.astype("uint8")).convert('L')
            im.save('./contra_feature_mask_result/images/gaborfilters/%d_cb3_%03d.png'%(epoch, o))
...
def saveParameters(net, epoch):
    path_rst = './contra_feature_mask_result/'
...
def saveLossACC(train_losses, val_losses, tr_cls_loss, tr_con_loss, tr_con_loss1, tr_con_loss2, tr_con_loss3, train_accuracy,
                val_cls_loss, val_con_loss, val_con_loss1, val_con_loss2, val_con_loss3, val_accuracy, bestacc):
    path_rst = './contra_feature_mask_result/'
    ...
    with open(os.path.join(path_rst,'tr_cls_loss.pickle'),'wb') as f:
        pickle.dump(tr_cls_loss, f)
    with open(os.path.join(path_rst,'tr_con_loss.pickle'),'wb') as f:
        pickle.dump(tr_con_loss, f)
    with open(os.path.join(path_rst,'tr_con_loss1.pickle'),'wb') as f:
        pickle.dump(tr_con_loss1, f)
    with open(os.path.join(path_rst,'tr_con_loss2.pickle'),'wb') as f:
        pickle.dump(tr_con_loss2, f)
    with open(os.path.join(path_rst,'tr_con_loss3.pickle'),'wb') as f:
        pickle.dump(tr_con_loss3, f)

    with open(os.path.join(path_rst,'val_losses.pickle'), 'wb') as f:
        pickle.dump(val_losses, f)
    with open(os.path.join(path_rst,'val_cls_loss.pickle'),'wb') as f:
        pickle.dump(val_cls_loss, f)
    with open(os.path.join(path_rst,'val_con_loss.pickle'),'wb') as f:
        pickle.dump(val_con_loss, f)
    with open(os.path.join(path_rst,'val_con_loss1.pickle'),'wb') as f:
        pickle.dump(val_con_loss1, f)
    with open(os.path.join(path_rst,'val_con_loss2.pickle'),'wb') as f:
        pickle.dump(val_con_loss2, f)
    with open(os.path.join(path_rst,'val_con_loss3.pickle'),'wb') as f:
        pickle.dump(val_con_loss3, f)
    ...
    with open(os.path.join(path_rst,'train_losses.txt'), 'w') as f:
        for v in train_losses:
            f.write(str(v)+'\n')
    with open(os.path.join(path_rst,'tr_cls_loss.txt'), 'w') as f:
        for v in tr_cls_loss:
            f.write(str(v)+'\n')
    with open(os.path.join(path_rst,'tr_con_loss.txt'), 'w') as f:
        for v in tr_con_loss:
            f.write(str(v)+'\n')
    with open(os.path.join(path_rst,'tr_con_loss1.txt'), 'w') as f:
        for v in tr_con_loss1:
            f.write(str(v)+'\n')
    with open(os.path.join(path_rst,'tr_con_loss2.txt'), 'w') as f:
        for v in tr_con_loss2:
            f.write(str(v)+'\n')
    with open(os.path.join(path_rst,'tr_con_loss3.txt'), 'w') as f:
        for v in tr_con_loss3:
            f.write(str(v)+'\n')
    ...
    with open(os.path.join(path_rst,'val_losses.txt'), 'w') as f:
        for v in val_losses:
            f.write(str(v)+'\n')
    with open(os.path.join(path_rst,'val_cls_loss.txt'), 'w') as f:
        for v in val_cls_loss:
            f.write(str(v)+'\n')
    with open(os.path.join(path_rst,'val_con_loss.txt'), 'w') as f:
        for v in val_con_loss:
            f.write(str(v)+'\n')
    with open(os.path.join(path_rst,'val_con_loss1.txt'), 'w') as f:
        for v in val_con_loss1:
            f.write(str(v)+'\n')
    with open(os.path.join(path_rst,'val_con_loss2.txt'), 'w') as f:
        for v in val_con_loss2:
            f.write(str(v)+'\n')
    with open(os.path.join(path_rst,'val_con_loss3.txt'), 'w') as f:
        for v in val_con_loss3:
            f.write(str(v)+'\n')
